var url = 'react/re/react'; // link file data
var url2 = 'react/'; // link ảnh
		...
		for (var i = 0; i < data.length; i++) {
			output += `
				<tr>
					...
					<td>${data[i].tda}</td>
					<td>${data[i].nd}</td>
					<td><button data-index="${i}" class="btn btn-dark nut"><i class="fab fa-react"></i></button></td>
				</tr>
			`;
		}
...
function loadData(index, url, url2) {
	var xhr = new XMLHttpRequest();
